...
function getBranchInfo($lnk) {
	$branchQry = $lnk->query("SELECT branchName, regional, location, branchNum FROM branchInfo.branches WHERE active  = 1");
	$arr       = [];
...
$rankingQry = $lnk->query("SELECT * FROM staffing.mrRankings WHERE mrYear = 2022 && mrID > 12164 ORDER BY mrBranch, mrPos");
